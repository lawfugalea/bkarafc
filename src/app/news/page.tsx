      ...
      <section className="max-w-7xl mx-auto px-6 py-14">
        ...
        {featured && (
          <div data-animate className="mb-10">
            <a
              ...
              className="card-interactive lg:flex gap-8 bg-surface group overflow-hidden"
            >
              ...
              <div className="p-6 flex flex-col justify-center">
                <span className={`text-xs font-semibold uppercase tracking-wider px-2.5 py-1 w-fit ${catBadgeClass(featured.category)}`}>
                  {catLabel[featured.category] ?? featured.category}
                </span>
                <h2 className="font-display font-bold italic text-white text-3xl mt-3 leading-tight group-hover:text-bka-gold transition-colors">
                  {featured.title}
                </h2>
                {featured.excerpt && (
              ...
              <div className="h-[3px] bg-bka-red scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left lg:hidden" />
            </a>
          </div>
        )}
        ...
        {rest.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {rest.map((item, i) => (
              <div key={item.slug.current} data-animate data-animate-delay={String(i * 75)}>
                <a
                  href={`/news/${item.slug.current}`}
                  className="card-interactive bg-surface group overflow-hidden"
                >

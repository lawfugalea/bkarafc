      ...
      <section className="max-w-5xl mx-auto px-6 py-14">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {tiers.map((tier) => (
            <div
              key={tier.name}
              className={`flex flex-col p-8 ${tier.highlight ? "bg-bka-red" : "bg-surface border border-white/10"}`}
            >
              ...
              <ul className="space-y-3 flex-1 mb-8">
                {tier.features.map((f) => (
                  <li key={f} className="flex items-start gap-2 text-sm text-white/75">
                    <span className={`mt-0.5 shrink-0 ${tier.highlight ? "text-white" : "text-bka-gold"}`}>✓</span>
                    {f}
                  </li>
                ))}
              </ul>
